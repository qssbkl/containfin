...
set<Key, Value> &set<Key, Value>::operator=(set<Key, Value> &&m) {
  if (this != &m) rb_tree<Key, Value>::swap(m.root_, m.fake_node_, m.size_);
  return *this;
}
...
std::vector<std::pair<typename set<Key, Value>::iterator, bool>>
set<Key, Value>::emplace(Args &&...args) {
  std::vector<std::pair<typename set<Key, Value>::iterator, bool>> v(
      sizeof...(args));
  size_t i = 0;
  ((v[i++] = (insert(args))), ...);
  return v;
