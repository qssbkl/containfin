...
template <typename T>
inline const T &s21::list<T>::front() const {
  return front_node_->value_;
}

template <typename T>
inline const T &s21::list<T>::back() const {
...
template <typename T>
inline unsigned long s21::list<T>::max_size() const {
  return SIZE_MAX / sizeof(Node) / 2;
}

template <typename T>
inline bool s21::list<T>::empty() const {
...
template <typename T>
void s21::list<T>::push_back(const T &val) {
...
void s21::list<T>::pop_back() {
  if (!empty()) {
    ...
    if (back_node_->prev_) back_node_->prev_->next_ = fake_node_;
    delete back_node_;
    if (fake_node_->prev_)
      back_node_ = fake_node_->prev_;
    else
...
template <typename T>
void s21::list<T>::push_front(const T &val) {
  // inline void s21::list<T>::push_front(const T &val) {
  ++size_;
  if (empty()) {
    front_node_ = back_node_ = fake_node_->prev_ =
        new Node{val, fake_node_, nullptr};
  } else {
    front_node_->prev_ = new Node;
    front_node_->prev_->next_ = front_node_;
    front_node_->prev_->prev_ = nullptr;
    front_node_->prev_->value_ = val;
    front_node_ = front_node_->prev_;
...
template <typename T>
inline void s21::list<T>::swap(list &other) {
...
void s21::list<T>::unique() {
  if (!empty()) {
    ...
    while (temp && temp->next_) {
      if (temp->next_ == fake_node_) break;
...
void s21::list<T>::splice(s21::list<T>::const_iterator pos,
  ...
  if (!other.empty()) {
    ...
    if (pos.pos_ == front_node_) front_node_ = other.front_node_;
    if (pos.pos_ == fake_node_) back_node_ = other.back_node_;
    other.back_node_ = other.front_node_ = other.fake_node_;
    other.size_ = 0;
  }
}
...
typename s21::list<T>::iterator s21::list<T>::insert(s21::list<T>::iterator it,
  ...
  if (temp) temp->next_ = it.pos_->prev_;
  if (it.pos_ == fake_node_) back_node_ = it.pos_->prev_;
  if (it.pos_ == front_node_) front_node_ = it.pos_->prev_;
...
template <typename T>
typename s21::list<T>::iterator s21::list<T>::emplace(const_iterator pos) {
  return iterator(const_cast<Node *>(pos.pos_));
}
