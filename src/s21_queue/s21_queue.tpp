...
template <typename T>
inline size_t s21::queue<T>::size() const {
  return size_;
}

template <typename T>
inline const T &s21::queue<T>::front() const {
  return front_node_->value_;
}

template <typename T>
inline const T &s21::queue<T>::back() const {
  return back_node_->value_;
}

template <typename T>
void s21::queue<T>::push(const T &val) {
  if (!empty())
...
template <typename T>
inline void s21::queue<T>::destroy() {
  while (!empty()) {
