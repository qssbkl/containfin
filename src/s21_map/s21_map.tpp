...
map<Key, Value> &map<Key, Value>::operator=(map<Key, Value> &&m) {
  if (&m != this) rb_tree<Key, Value>::swap(m.root_, m.fake_node_, m.size_);
  return *this;
}
...
void map<Key, Value>::merge(map<Key, Value> &other) {
  ...
  for (auto i = other.begin(); i != other.end(); ++i) l.push_back(i.iter_);
  ...
  for (auto i = l.begin(); i != l.end(); ++i) other.insert_existing(*i);
}
