...
TEST(Constructor, cons9) {
  ...
  for (int count = 0; (int)map_a.size() > count && count != 50; ++count) {
...
TEST(Size, size1) {
  map<int, char> map_a{{3, 2}, {53, 3}, {51, 3}};
  Map<int, char> map_b{{3, 2}, {53, 3}, {51, 3}};
...
TEST(funcs, emplace_normal_value) {
  ...
  try {
    ...
    ASSERT_STREQ("Index out of range", expected.what());
  }
