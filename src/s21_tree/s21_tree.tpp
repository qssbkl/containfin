...
rb_tree<Key, Value>::rb_tree()
    : size_(0),
      root_(new Node<const Key, Value>{Key{}, Value{}}),
      fake_node_(root_) {}
...
void rb_tree<Key, Value>::destroy(Node<const Key, Value> *node) {
  if (fake_node_->parent) fake_node_->parent->right = nullptr;
  ...
  if (root_ != fake_node_) {
    ...
    while (node_ptr && !res) {
      if (val > node_ptr->value.first) {
        ...
        if (node_ptr == fake_node_) break;
        res = true;
      }
    }
  }
  if (!res) node_ptr = fake_node_;
}
...
void rb_tree<Key, Value>::rb_delete(Node_type *deleting_node) {
  ...
  if (deleting_node && deleting_node != fake_node_) {
    ...
    --size_;
    if (original_color == BLACK) balance_fixup_delete(ptr, ptr_parent);
  }
...
template <typename Key, typename Value>
Node<const Key, Value> *rb_tree<Key, Value>::transplant(
    Node_type *&node, Node_type *&child_node) {
  if (!node->parent) {
    root_ = child_node;
...
void rb_tree<Key, Value>::insert_any_existing(Node_type *&new_node) {
  ...
  while (pointer && pointer != fake_node_) {
    par = pointer;
    if (get_pointer(pointer, new_node->value.first)) break;
  }
  insertion(new_node, par, pointer);
}
...
template <typename Key, typename Value>
inline void rb_tree<Key, Value>::change_node_relations(
    Node<const Key, Value> *new_node) {
...
Node<const Key, Value> *rb_tree<Key, Value>::minimum() const {
  ...
  if (!empty()) {
    while (iter && iter->left != nullptr) iter = iter->left;
  }
  return iter;
}
...
Node<const Key, Value> *rb_tree<Key, Value>::find_first(Key key,
  ...
  if (root_ != fake_node_) {
    if (start) {
      ...
      if (start && start->left) temp = find_first(key, start->left);
      if (temp && temp->value.first >= key && temp != start) return temp;
    }
  }
